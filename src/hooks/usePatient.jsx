...
import { patientAPI, appointmentAPI, doctorsAPI, medicalRecordsAPI, prescriptionsAPI, notificationsAPI, messagesAPI, } from '../services/patientAPI';
...
export const useChangePassword = () => {
    return useMutation({
        mutationFn: ({ currentPassword, newPassword }) => patientAPI.changePassword(currentPassword, newPassword),
    });
};
// Dashboard Query
...
export const usePatientAppointments = (status) => {
    return useQuery({
...
export const useCancelAppointment = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (appointmentId) => appointmentAPI.cancelAppointment(appointmentId),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['patient-appointments'] });
            queryClient.invalidateQueries({ queryKey: ['patient-dashboard'] });
        },
...
// Doctors Query
export const useDoctors = (specialization) => {
    return useQuery({
...
export const useSendMessage = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (data) => messagesAPI.sendMessage(data.conversationId, data.receiverId, data.message),
        onSuccess: () => {
...
export const useStartConversation = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (doctorId) => messagesAPI.startConversation(doctorId),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['conversations'] });
        },
    });
