...
import { Calendar, Clock, Phone, CheckCircle, XCircle, AlertCircle, Edit2, Eye } from "lucide-react";
...
const DoctorAppointments = () => {
    ...
    return (<div className="space-y-6 bg-[#f3f7fc] min-h-screen -m-6 p-6">
      {message && (<div className="p-4 rounded-2xl bg-green-50 text-green-800 border border-green-200">
          {message}
        </div>)}
      ...
      <div className="bg-white rounded-2xl shadow-md p-4">
        <div className="flex gap-3 flex-wrap">
          {statuses.map((status) => (<button key={status} onClick={() => {
                setSelectedStatus(status);
                setPage(1);
            }} className={`px-5 py-3 rounded-xl font-medium transition text-lg ${selectedStatus === status
      ...
      <div className="bg-white rounded-2xl shadow-md overflow-hidden min-h-[240px]">
        {appointments.length === 0 ? (<div className="p-16 text-center text-gray-500 flex flex-col items-center justify-center min-h-[240px]">
            ...
            <table className="w-full">
              ...
              <tbody>
                {appointments.map((apt) => (<tr key={apt._id} className="border-b hover:bg-gray-50 transition">
                    ...
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(apt.status)}`}>
      ...
      {/* Detail Modal */}
      {showDetailModal && selectedAppointment && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-2xl w-full max-h-96 overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <p>
                      ...
                      <span className={`inline-block px-2 py-1 rounded font-medium text-sm ${getStatusColor(selectedAppointment.status)}`}>
              ...
              {["pending", "approved"].includes(selectedAppointment.status) && (<button onClick={() => {
      ...
      {/* Update Modal */}
      {updateModal && selectedAppointment && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-md w-full">
            <div className="p-6 border-b flex items-center justify-between">
              <h3 className="text-xl font-bold text-gray-900">Update Appointment</h3>
              <button onClick={() => setUpdateModal(false)} className="text-gray-500 hover:text-gray-700 text-2xl">
                ×
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <select value={updateData.status} onChange={(e) => setUpdateData({ ...updateData, status: e.target.value })} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
              ...
              <div>
                ...
                <textarea value={updateData.notes} onChange={(e) => setUpdateData({ ...updateData, notes: e.target.value })} rows={3} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Add any notes..."/>
              </div>

              <div className="flex gap-3">
                <button onClick={() => setUpdateModal(false)} className="flex-1 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 font-medium">
                  Cancel
                </button>
                <button onClick={handleStatusUpdate} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium">
                  Update
                </button>
              </div>
            </div>
