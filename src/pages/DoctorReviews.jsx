    ...
    return (<div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-center">
          <div className="flex items-center justify-center gap-2">
            {[...Array(5)].map((_, i) => (<Star key={i} size={24} className={i < Math.round(averageRating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}/>))}
          </div>
      ...
      <div className="space-y-4">
        {loading ? (<div className="p-8 text-center">
          ...
          </div>) : reviews.length === 0 ? (<div className="p-8 text-center text-gray-500 bg-white rounded-lg">
            <MessageCircle size={48} className="mx-auto mb-2 opacity-50"/>
            <p>No reviews yet</p>
          </div>) : (reviews.map((review) => (<div key={review._id} className="bg-white rounded-lg shadow-md p-6">
