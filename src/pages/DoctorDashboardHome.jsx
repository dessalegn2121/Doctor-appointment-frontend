...
import { Users, Calendar, CheckCircle, Clock, Pill, Bell, TrendingUp, Activity, AlertCircle, ArrowUpRight, ArrowDownRight } from "lucide-react";
    ...
    return (<div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {statCards.map((stat) => {
            ...
            return (<div key={stat.id} className={`${stat.bgColor} border-2 ${stat.borderColor} rounded-xl p-6 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1`}>
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    {stat.trendUp ? (<ArrowUpRight className="text-green-600" size={16}/>) : (<ArrowDownRight className="text-red-600" size={16}/>)}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8 hover:shadow-md transition-shadow">
        ...
        ].map((action) => (<Link key={action.to} to={action.to} className={`${action.color} text-white rounded-xl px-4 py-4 text-center font-semibold shadow-sm hover:shadow-md hover:opacity-95 transition`}>
              {action.label}
            </Link>))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
          ...
          {weeklyChart.length > 0 ? (<ResponsiveContainer width="100%" height={300}>
            ...
            </ResponsiveContainer>) : (<div className="flex items-center justify-center py-12">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
          ...
          {monthlyChart.length > 0 ? (<ResponsiveContainer width="100%" height={300}>
              <LineChart data={monthlyChart}>
                ...
                <Legend />
                <Line type="monotone" dataKey="completed" stroke="#10b981" strokeWidth={3} dot={{ fill: "#10b981", r: 5 }} activeDot={{ r: 7 }}/>
              </LineChart>
            </ResponsiveContainer>) : (<div className="flex items-center justify-center py-12">
              <p className="text-gray-500">No data available</p>
            </div>)}
      ...
      {/* Recent Appointments */}
      {overview?.recentAppointments && overview.recentAppointments.length > 0 && (<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8 hover:shadow-md transition-shadow">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {overview.recentAppointments.map((apt) => (<tr key={apt._id} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold ${apt.status === "completed"
      ...
      {overview?.recentNotifications && overview.recentNotifications.length > 0 && (<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
          <h2 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
            <Bell className="text-orange-600" size={24}/>
            Recent Notifications
          </h2>
          <div className="space-y-4">
            {overview.recentNotifications.map((notif) => (<div key={notif._id} className="p-4 bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg border border-blue-200 hover:shadow-md transition-shadow">
                <div className="flex items-start gap-4">
