      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredDoctors.map((doctor: any) => (
            <div key={doctor._id} className="bg-white rounded-lg shadow hover:shadow-lg transition overflow-hidden">
              {doctor.profileImage && (
                <img
              ...
              <div className="p-6">
                ...
                <div className="flex space-x-3">
                  <Link
                    to={`/patient/appointments/book?doctorId=${doctor._id}`}
                    className="flex-1 text-center bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 transition"
                  >
                    Book Now
                  </Link>
                  <Link
                    to={`/patient/messages?doctorId=${doctor._id}`}
                    className="flex-1 text-center bg-blue-100 text-blue-600 py-2 rounded-lg hover:bg-blue-200 transition"
                  >
                    Message
                  </Link>
                </div>
