    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white rounded-lg p-8">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Link to="/patient/profile" className="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-blue-700 transition hover:bg-blue-50">
            <User size={18}/>
            View Profile
          </Link>
          <button type="button" onClick={openBookingForm} className="inline-flex items-center gap-2 rounded-lg border border-white/30 px-4 py-2 text-sm font-semibold text-white transition hover:bg-white/10">
            <Calendar size={18}/>
            Book Appointment
          </button>
        </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {quickActions.map((action) => {
            ...
            if (isActionButton) {
                return (<button key={action.label} type="button" onClick={action.action} className={`flex items-center justify-between rounded-xl px-4 py-4 font-semibold transition ${action.color}`}>
                  <span>{action.label}</span>
                  <Icon size={20}/>
                </button>);
            }
            return (<Link key={action.to} to={action.to} className={`flex items-center justify-between rounded-xl px-4 py-4 font-semibold transition ${action.color}`}>
                <span>{action.label}</span>
                <Icon size={20}/>
              </Link>);
        })}
      ...
      {showBookingForm && (<div ref={bookingSectionRef} id="book-appointment" className="bg-white rounded-lg shadow p-6 border border-blue-100">
          <div className="mb-4 flex items-center justify-between gap-4">
            ...
            <button type="button" onClick={() => setShowBookingForm(false)} className="rounded-lg border border-gray-300 px-3 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50">
              Close
            </button>
          </div>
          <BookAppointmentPage />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="space-y-3">
            <button type="button" onClick={openBookingForm} className="block w-full rounded-lg bg-blue-600 py-3 text-center text-white transition hover:bg-blue-700">
              📅 Book Appointment
            </button>
            <Link to="/patient/doctors" className="block w-full text-center bg-blue-100 text-blue-600 py-3 rounded-lg hover:bg-blue-200 transition">
              👨‍⚕️ View Doctors
            </Link>
            <Link to="/patient/medical-records" className="block w-full text-center bg-blue-100 text-blue-600 py-3 rounded-lg hover:bg-blue-200 transition">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          {dashboard?.upcomingAppointments?.length > 0 ? (<div className="space-y-3">
              {dashboard.upcomingAppointments.slice(0, 3).map((apt) => (<div key={apt._id} className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
                  <div>
              ...
              <Link to="/patient/appointments" className="text-center text-blue-600 hover:text-blue-800 font-semibold text-sm">
                View All →
              </Link>
            </div>) : (<p className="text-gray-500 text-center py-6">No upcoming appointments</p>)}
        </div>
      </div>
    </div>);
};
