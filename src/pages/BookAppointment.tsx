...
import { bookAppointmentSchema, BookAppointmentFormData } from '../schemas/validationSchemas';
...
const BookAppointment: React.FC = () => {
  ...
  const departments = Array.isArray(departmentsData?.data)
    ...
    : Array.isArray((departmentsData as any)?.data?.data)
      ? (departmentsData as any).data.data
      : [];
  ...
  const selectedDepartment = departments.find((dept: any) => dept._id === selectedDepartmentId);

  useEffect(() => {
    if (!doctorIdParam || !doctorDetailsData?.data) return;

    const doctor = doctorDetailsData.data;
    const matchedDepartment = departments.find(
      (dept: any) => dept.name.trim().toLowerCase() === String(doctor.department || '').trim().toLowerCase()
    );
  ...
  const doctors = Array.isArray(doctorsPayload)
    ? doctorsPayload
    : Array.isArray((doctorsPayload as any)?.doctors)
      ? (doctorsPayload as any).doctors
      : Array.isArray((doctorsQuery.data as any)?.data?.doctors)
        ? (doctorsQuery.data as any).data.doctors
        : [];

  const selectedDoctor = doctors.find(
    (doc: any) => doc._id === selectedDoctorId
  );
