              ...
              {conversations.map((conversation: any) => (
                <button
                  ...
                  className={`w-full text-left p-4 border-b border-gray-100 hover:bg-blue-50 transition ${
                    selectedConversationId === conversation._id ? 'bg-blue-100 border-l-4 border-l-blue-600' : ''
                  }`}
                >
            ...
            <>
              ...
              <div className="p-4 border-b border-gray-200 bg-blue-50">
                <button
                  ...
                  className="flex items-center space-x-2 text-blue-600 mb-2 lg:hidden"
                >
                  <ArrowLeft size={20} />
                  <span>Back</span>
                </button>
                {conversations.find((c: any) => c._id === selectedConversationId) && (
                  <div>
                    <p className="font-bold text-gray-800">
                      {conversations.find((c: any) => c._id === selectedConversationId)?.doctorId?.name}
                    </p>
                    <p className="text-sm text-gray-600">
                      {conversations.find((c: any) => c._id === selectedConversationId)?.doctorId?.specialization}
                    </p>
                  </div>
                )}
              </div>
                        ...
                        >
                          ...
                          <p
                            className={`text-xs mt-1 ${
                              message.senderType === 'patient' ? 'text-blue-100' : 'text-gray-600'
                            }`}
                          >
