...
const PatientMedicalRecords = () => {
    ...
    return (<div className="space-y-6">
      ...
      {isLoading ? (<div className="flex items-center justify-center py-12">
        ...
        </div>) : records.length > 0 ? (<div className="space-y-4">
          {records.map((record) => (<div key={record._id} className="bg-white rounded-lg shadow p-6 hover:shadow-lg transition">
              ...
              {record.medications && record.medications.length > 0 && (<div className="mb-4">
                  <p className="text-sm text-gray-600 font-semibold">Medications</p>
                  <ul className="list-disc list-inside space-y-1">
                    {record.medications.map((med, idx) => (<li key={idx} className="text-gray-800">{med}</li>))}
                  </ul>
                </div>)}

              {record.notes && (<div className="mb-4">
                  <p className="text-sm text-gray-600 font-semibold">Notes</p>
                  <p className="text-gray-800">{record.notes}</p>
                </div>)}

              {record.attachments && record.attachments.length > 0 && (<div className="flex space-x-2">
                  {record.attachments.map((attachment, idx) => (<a key={idx} href={attachment} download className="flex items-center space-x-2 text-blue-600 hover:text-blue-800 transition">
                      <Download size={18}/>
